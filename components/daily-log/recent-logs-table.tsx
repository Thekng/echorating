    ...
    <div className="space-y-3">
                ...
                <div className="flex items-center justify-end gap-2">
                  ...
                  {canDelete ? (
                    <button
                      ...
                      className="inline-flex h-8 w-8 items-center justify-center rounded-md border border-destructive/30 text-destructive hover:bg-destructive/10 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring"
      ...
      {logToDelete && (
        <ConfirmDialog
          ...
          title="Delete log entry?"
          description={`Delete log for ${logToDelete.user_name} on ${formatDateShort(logToDelete.report_date)}?`}
          confirmText="Delete"
          isLoading={isDeleting}
          onConfirm={() => {
            startDeleteTransition(async () => {
              const formData = new FormData()
              formData.set('entryId', logToDelete.id)
