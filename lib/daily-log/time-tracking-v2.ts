...
export class ConflictResolution {
   ...
   */
  static async fetchLatestEntry(supabase: unknown, entryId: string) {
    const { data, error } = await supabase
      .from('daily_entries')
      .select(`
        *,
   ...
   */
  static mergeEntries(local: Record<string, unknown>, remote: Record<string, unknown>, base?: unknown): Record<string, unknown> {
    const merged = { ...remote }
    ...
    for (const value of local.entry_values || []) {
      const remoteValue = (remote.entry_values || []).find(
        (v: Record<string, unknown>) => v.metric_id === value.metric_id
      )
