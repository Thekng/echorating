...
export class ConflictResolution {
   ...
   */
  static async fetchLatestEntry(supabase: Record<string, any>, entryId: string) {
    const { data, error } = await supabase
      .from('daily_entries')
      .select(`
        *,
   ...
   */
  static mergeEntries(local: Record<string, any>, remote: Record<string, any>, _base: Record<string, any>): any {
    ...
    for (const value of local.entry_values || []) {
      const remoteValue = (remote.entry_values || []).find(
        (v: Record<string, any>) => v.metric_id === value.metric_id
      )
